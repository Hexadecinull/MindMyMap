...
MM.Item = function() {
	...
	const item = this._dom.node;
	this._dom.toggle.addEventListener("click", this);
	this._dom.notes.addEventListener("click", function () {
		MM.App.notes.toggle('item');
		item.click();
	});
}
...
MM.Clipboard.paste = function(targetItem) {
	setTimeout(function() {
		...
		if (this._item && pasted == MM.Format.Plaintext.to(this._item.toJSON())) { /* pasted a previously copied/cut item */
			...
			var pastedText = [span.textContent || span.innerText].toString().replace(/ +/g,' ');
			this._pastePlaintext(pastedText, targetItem);
		}
...
MM.Command.Finish.execute = function(event) {
	let valid = false
	if (event && event.keyCode == 13 && document.querySelector('#port').contains(event.target)) {
		valid = true
	} else if (!event) {
		valid = true
	}
	if (MM.App._admin === true && valid) {
...
MM.Format.Plaintext._serializeItem = function(item, depth) {
	...
	var icon = '';
	if (item.icon && item.icon !== null && item.icon !== '') {
		icon = item.icon + ' ';
	}
	var status = '';
	if (item.status && item.status === 'yes') {
		status = ' ✔️';
	} else if (item.status && item.status === 'no') {
		status = ' ❌';
	}
	var link = '';
	...
	var text = DOMPurify.sanitize(item.text);
	text = text.replace(/\n/g, " ").replace(/<br>/gi, " ");
	var temp = document.createElement('div');
	temp.innerHTML = text;
	text = temp.textContent;
	if (item.notes && item.notes !== null && item.notes !== '') {
		var notes = DOMPurify.sanitize(item.notes);
		notes = notes.replace(/<br>/gi, " ").replace(/<li>/gi, "<li> • ").replace(/<\/ul>/gi, " ").replace(/<\/ol>/gi, " ");
		var tempHtml = document.createElement('div');
		tempHtml.innerHTML = notes;
		notes = tempHtml.textContent;
		lines.unshift(prefix + icon + text.trim() + status + link + ' (' + notes.trim() + ')');
	} else {
		lines.unshift(prefix + icon + text.trim() + status + link);
	}
...
MM.UI.Notes.prototype.toggle = function(source) {
	window.postMessage({ type: 'notes', source: source });
}
...
MM.App = {
	...
	_syncPort: function() {
		...
		if (document.getElementById('header') && document.getElementById('outils')) {
			windowInnerHeight = window.innerHeight - (document.getElementById('header').offsetHeight + document.getElementById('outils').offsetHeight)
		}
		this.portSize = [window.innerWidth, windowInnerHeight];
